...
def get_vaccines(data):
    return data[data.county == "Santa Barbara"][
        [
            "date",
            "population",
...
def get_deaths(data):
    ...
    return data[["date", "area", "pct"]].rename({"area": "county"}, axis=1)
...
def get_ages(data):
    ageGroups = (
        data[data.demographic_category == "Age Group"].reset_index(drop=True).dropna()
    )

    ageGroups["partialPct"] = (
        ageGroups["cumulative_at_least_one_dose"] / ageGroups["est_population"]
    )

    return ageGroups[["date", "partialPct", "demographic_value"]].sort_values(
        by=["demographic_value", "date"]
    )
...
def get_races(data):
    raceGroups = (
        data[data.demographic_category == "Race/Ethnicity"]
        .reset_index(drop=True)
        .dropna()
    )

    raceGroups["partialPct"] = (
        raceGroups["cumulative_at_least_one_dose"] / raceGroups["est_population"]
    )
    ...
    raceGroups = raceGroups[
        raceGroups.demographic_value != "Native Hawaiian or Other Pacific Islander"
    ]
    raceGroups = raceGroups[
        raceGroups.demographic_value != "American Indian or Alaska Native"
    ]

    raceGroups.demographic_value = raceGroups.demographic_value.replace(
...
def get_zip_codes(data):
    zip_codes_select = np.array(
        "93067 93441 93111 93108 93109 93013 93103 93101 93110 93105 93440 93460 93463 93434 93427 93454 93117 93458 93254 93455 93436 93429".split(
            " "
        )
    ).astype("int")

    data = data[
        [(zc in zip_codes_select) for zc in data.zip_code_tabulation_area]
    ].copy()
    ...
    return (
        ...
        .rename({"zip_code_tabulation_area": "zip"}, axis=1)
        .reset_index(drop=True)
    )
...
def fetch_data():
    ...
    vaccines.county = np.where(
        vaccines.county == "All CA Counties", "California", vaccines.county
    )
    cases_deaths.area = np.where(
        cases_deaths.area == "All CA Counties", "California", cases_deaths.area
    )

    combined = vaccines.astype({"county": "string", "date": np.datetime64}).merge(
    ...
    vaccines_demographics = vaccines_demographics[
        vaccines_demographics["county"] == "Santa Barbara"
    ]
    vaccines_demographics["date"] = pd.to_datetime(
        vaccines_demographics["administered_date"]
    )
